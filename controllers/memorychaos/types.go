...
func (r *Reconciler) Reconcile(req ctrl.Request, chaos *v1alpha1.MemoryChaos) (ctrl.Result, error) {
	r.Log.Info("Reconciling memorychaos")
	...
	if scheduler == nil && duration == nil {
		return r.commonChaos(chaos, req)
	} else if scheduler != nil && duration != nil {
		return r.scheduleChaos(chaos, req)
	}
...
func (r *Reconciler) commonChaos(timechaos *v1alpha1.MemoryChaos, req ctrl.Request) (ctrl.Result, error) {
	cr := common.NewReconciler(r, r.Client, r.Log)
	return cr.Reconcile(req)
}

func (r *Reconciler) scheduleChaos(timechaos *v1alpha1.MemoryChaos, req ctrl.Request) (ctrl.Result, error) {
...
func (r *Reconciler) Apply(ctx context.Context, req ctrl.Request, chaos reconciler.InnerObject) error {
	memchaos, ok := chaos.(*v1alpha1.MemoryChaos)
	if !ok {
		err := errors.New("chaos is not memorychaos")
		r.Log.Error(err, "chaos is not MemoryChaos", "chaos", chaos)
		return err
	}

	pods, err := utils.SelectAndGeneratePods(ctx, r.Client, &memchaos.Spec)
	...
	err = r.applyAllPods(ctx, pods, memchaos)
	if err != nil {
		r.Log.Error(err, "failed to apply chaos on all pods")
		return err
	}

	memchaos.Status.Experiment.Pods = []v1alpha1.PodStatus{}
	...
	for _, pod := range pods {
		ps := v1alpha1.PodStatus{
			...
			Message:   fmt.Sprintf(memorychaosMsg, memchaos.Spec.Quota),
		}

		memchaos.Status.Experiment.Pods = append(memchaos.Status.Experiment.Pods, ps)
	}
	r.Event(memchaos, v1.EventTypeNormal, utils.EventChaosInjected, "")
	return nil
}
...
func (r *Reconciler) recoverPod(ctx context.Context, pod *v1.Pod, chaos *v1alpha1.TimeChaos) error {
	...
	c, err := utils.CreateGrpcConnection(ctx, r.Client, pod, "65533")
	if err != nil {
		return err
	}
	defer c.Close()
