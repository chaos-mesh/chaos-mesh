...
func (r *endpoint) Apply(ctx context.Context, req ctrl.Request, chaos v1alpha1.InnerObject) error {
	...
	pods, err := selector.SelectAndFilterPods(ctx, r.Client, r.Reader, &kernelChaos.Spec, config.ControllerCfg.ClusterScoped, config.ControllerCfg.TargetNamespace, config.ControllerCfg.EnableFilterNamespace)
	if err != nil {
...
func CreateBPFKIConnection(ctx context.Context, c kubeclient.Client, pod *v1.Pod) (*grpc.ClientConn, error) {
	daemonIP, err := utils.FindDaemonIP(ctx, c, pod)
	if err != nil {
		return nil, err
	}

	return grpcUtils.Builder().WithDefaultTimeout().Address(daemonIP).Port(config.ControllerCfg.BPFKIPort).TryTLSFromFiles(config.ControllerCfg.TLSConfig.ChaosMeshCACert, config.ControllerCfg.TLSConfig.ChaosDaemonClientCert, config.ControllerCfg.TLSConfig.ChaosDaemonClientKey).Build()
}
