...
func New(cfg Config, metrics *metrics.ChaosControllerManagerMetricsCollector) (*K8sConfigMapWatcher, error) {
	...
	if strings.TrimSpace(c.TemplateNamespace) == "" {
		...
		if err != nil {
			if os.IsNotExist(err) {
				return nil, errors.Errorf("%s: maybe you should specify ----template-namespace if you are running outside of kubernetes", err.Error())
			}
			return nil, err
		}
	...
	if err = validate(&c); err != nil {
		return nil, errors.Errorf("validation failed for K8sConfigMapWatcher: %s", err.Error())
	}
...
func (c *K8sConfigMapWatcher) Watch(notifyMe chan<- interface{}, stopCh <-chan struct{}) error {
	...
	if err != nil {
		return errors.Errorf("unable to create template watcher (possible serviceaccount RBAC/ACL failure?): %s", err.Error())
	}
	...
	if err != nil {
		return errors.Errorf("unable to create config watcher (possible serviceaccount RBAC/ACL failure?): %s", err.Error())
	}
	defer func() {
		configWatcher.Stop()
