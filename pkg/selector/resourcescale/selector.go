...
type ResourceSpec struct {
	v1alpha1.ResourceScaleSelector
...
func (impl *SelectImpl) Select(ctx context.Context, selector *v1alpha1.ResourceScaleSelector) ([]*ResourceSpec, error) {
	...
	specs := &ResourceSpec{ResourceScaleSelector: *selector}
	...
	case v1alpha1.ResourceTypeDeployment:
		...
		specs.InitialReplicas = dep.Status.Replicas
	case v1alpha1.ResourceTypeReplicaSet:
		...
		specs.InitialReplicas = rs.Status.Replicas
	case v1alpha1.ResourceTypeStatefulSet:
		...
		specs.InitialReplicas = sts.Status.Replicas
	default:
	...
	// Set apply replicas to default 0 if not provided
	if specs.ApplyReplicas == nil {
		applyReplicas := int32(0)
		specs.ApplyReplicas = &applyReplicas
	}

	// Set recover replicas to default if not provided
	if specs.RecoverReplicas == nil {
		specs.RecoverReplicas = &specs.InitialReplicas
	}
