...
func GetPods(ctx context.Context, chaosName string, status v1alpha1.ChaosStatus, selectorSpec v1alpha1.SelectorSpec, c client.Client) ([]v1.Pod, []v1.Pod, error) {
	...
	pods, err := selector.SelectPods(ctx, c, c, selectorSpec, selector.FromControllerConfig(ctrlconfig.ControllerCfg))
	if err != nil {
	...
	for _, pod := range pods {
		...
		daemons, err := selector.SelectPods(ctx, c, nil, daemonSelector, selector.FromControllerConfig(ctrlconfig.ControllerCfg))
		if err != nil {
...
func checkConnForCtrlAndDaemon(ctx context.Context, daemons []v1.Pod, c *ClientSet) error {
	...
	ctrlMgrs, err := selector.SelectPods(ctx, c.CtrlCli, c.CtrlCli, ctrlSelector, selector.FromControllerConfig(ctrlconfig.ControllerCfg))
	if err != nil {
