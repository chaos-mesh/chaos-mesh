...
var _ = ginkgo.Describe("[Basic]", func() {
	...
	ginkgo.Context("[TimeChaos]", func() {
		...
		ginkgo.JustBeforeEach(func() {
			...
			err = util.WaitDeploymentReady(context.Background(), "timer", ns, kubeCli)
	...
	ginkgo.Context("[IOChaos]", func() {
		...
		ginkgo.JustBeforeEach(func() {
			...
			err = util.WaitDeploymentReady(context.Background(), "io-test", ns, kubeCli)
	...
	ginkgo.Context("[HTTPChaos]", func() {
		...
		ginkgo.JustBeforeEach(func() {
			...
			err = util.WaitDeploymentReady(context.Background(), "http-test", ns, kubeCli)
	...
	ginkgo.Context("[NetworkChaos]", func() {
		...
		ginkgo.JustBeforeEach(func() {
			...
			for index := 0; index < 4; index++ {
				...
				err = util.WaitDeploymentReady(context.Background(), name, ns, kubeCli)
