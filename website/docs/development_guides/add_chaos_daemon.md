...
After [Develop a new chaos](dev_hello_world.md), we add a new chaos type `HelloWorldChaos`, it will print `hello world` in `chaos-controller-manager`. Let's do more thing for this chaos type: get the hostname of selected pods, and print `hello` to these Pod in `chaos-daemon`.

## Add selector for HelloWorldChaos

Since we need to select some pods to do `HelloWorld` chaos, we need to add some filed for `HelloWorldChaos`:
...
We need to implement `InnerObject` interface for `HelloWorldChaos`, just do nothing in these functions:
...
And then we need to implement `SelectSpec` interface for `HelloWorldSpec`:
...
## Add new GRPC interface

Add new GRPC interface for `chaos-controller-manager` and `chaos-daemon`, so that `chaos-daemon` can receive the request and then do some chaos action in node.
...
And the definition of these request is as follows:
...
Don't forget update golang code generated by this proto file:
...
## Implement GRPC client in `chaos-controller-manager`

Create a dir named `helloworld` under controllers, and create a golang file `type.go` under `helloworld` dir, fill this file with content below:
...
## Run chaos
    ...
    2. Deploy pods for test
        ...
        This comamnd will deploy two pods in `busybox` namespace.

    3. Create chaos YAML file
