...
function e2e::image_load() {
    ...
    if [ "$PROVIDER" == "kind" ]; then
        ...
        # ghcr.io/chaos-mesh/chaos-coredns:v0.2.4, nginx:latest and gcr.io/google-containers/pause:latest is required for test
        # we suppose that you could pull this image on your host docker
        echo "info: load images ghcr.io/chaos-mesh/chaos-coredns:v0.2.4, nginx:latest and gcr.io/google-containers/pause:latest"
        docker pull ghcr.io/chaos-mesh/chaos-coredns:v0.2.4
        docker pull nginx:latest
        docker pull gcr.io/google-containers/pause:latest
        $KIND_BIN load docker-image --name $CLUSTER ghcr.io/chaos-mesh/chaos-coredns:v0.2.4 --nodes $(hack::join ',' ${nodes[@]})
