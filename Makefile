...
DOCKER_BUILD_ARGS := --build-arg TARGET_PLATFORM=$(if $(TARGET_PLATFORM),$(TARGET_PLATFORM),amd64) --build-arg HTTP_PROXY=${HTTP_PROXY} --build-arg HTTPS_PROXY=${HTTPS_PROXY} --build-arg UI=${UI} --build-arg LDFLAGS="${LDFLAGS}" --build-arg CRATES_MIRROR="${CRATES_MIRROR}"
