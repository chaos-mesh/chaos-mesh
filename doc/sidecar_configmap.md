...
* **volumeMounts**: defines the new volumeMounts or overwrite the old volumeMounts of the each containers in target pod.
...
`chaosfs` uses [fuse libary](https://github.com/hanwen/go-fuse) and [fusermounto](https://www.kernel.org/doc/Documentation/filesystems/fuse.txt) tools to implement a fuse-daemon service 
...
The following config will inject `chaosfs` container to target pod and will start a `chaosfs` process in this container.
...
`chaos-scripts` container is used to inject some scripts to target pod, include [wait-fuse.sh](https://github.com/pingcap/chaos-mesh/blob/master/hack/wait-fuse.sh).
...
`/tmp/scripts` is an [emptyDir volume](https://kubernetes.io/docs/concepts/storage/volumes/#emptydir) to shares the scripts with all containers of pod.
